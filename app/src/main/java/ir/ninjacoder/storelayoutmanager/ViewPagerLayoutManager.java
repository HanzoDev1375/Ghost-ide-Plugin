...
/**
 * An implementation of {@link RecyclerView.LayoutManager} which behaves like view pager. Please
 * make sure your child view have the same size.
 */
...
public abstract class ViewPagerLayoutManager extends LinearLayoutManager {
  ...
  /**
   * Many calculations are made depending on orientation. To keep it clean, this interface helps
   * {@link LinearLayoutManager} make those decisions. Based on {@link #mOrientation}, an
   * implementation is lazily created in {@link #ensureLayoutState} method.
   */
  ...
  /**
   * This keeps the final value for how LayoutManager should start laying out views. It is
   * calculated by checking {@link #getReverseLayout()} and View's layout direction. {@link
   * #onLayoutChildren(RecyclerView.Recycler, RecyclerView.State)} is run.
   */
   ...
   * it below api 21 or you can just setElevation in {@link #setItemViewProperty(View, float)}
   */
   ...
   *
   * <p>If you are using a {@link RecyclerView.RecycledViewPool}, it might be a good idea to set
   * this flag to <code>true</code> so that views will be available to other RecyclerViews
   * immediately.
   *
   * <p>Note that, setting this flag will result in a performance drop if RecyclerView is restored.
   *
  ...
  /**
   * Sets the orientation of the layout. {@link ViewPagerLayoutManager} will do its best to keep
   * scroll position.
   *
  ...
  /**
   * Calculates the view layout order. (e.g. from end to start or start to end) RTL layout support
   * is applied automatically. So if layout is RTL and {@link #getReverseLayout()} is {@code true},
   * elements will be laid out starting from left.
   */
   ...
   * out before it etc.
   *
   * <p>For horizontal layouts, it depends on the layout direction. When set to true, If {@link
  ...
  public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (mDistanceToBottom == INVALID_SIZE) {
      mSpaceInOther =
          (mOrientationHelper.getTotalSpaceInOther() - mDecoratedMeasurementInOther) / 2;
    } else {
      mSpaceInOther =
          mOrientationHelper.getTotalSpaceInOther()
  ...
  public boolean onAddFocusables(
    ...
    if (recyclerView.hasFocus()) {
      ...
      if (movement != DIRECTION_NO_WHERE) {
        final int targetPosition =
            movement == DIRECTION_BACKWARD ? currentPosition - 1 : currentPosition + 1;
  ...
  private int computeScrollOffset() {
    ...
    if (!mSmoothScrollbarEnabled) {
      return !mShouldReverseLayout
          ? getCurrentPosition()
          : getItemCount() - getCurrentPosition() - 1;
    }
  ...
  private void layoutItems(RecyclerView.Recycler recycler) {
    ...
    final int currentPos =
        mShouldReverseLayout ? -getCurrentPositionOffset() : getCurrentPositionOffset();
    ...
    for (int i = start; i < end; i++) {
      if (useMaxVisibleCount() || !removeCondition(getProperty(i) - mOffset)) {
        ...
        final float orderWeight =
            mEnableBringCenterToFront ? setViewElevation(scrap, targetOffset) : adapterPosition;
  ...
  /**
   * when the target offset reach this, the view will be removed and recycled in {@link
  ...
  public int getOffsetToCenter() {
    ...
    return (int)
        ((getCurrentPosition() * (!mShouldReverseLayout ? mInterval : -mInterval) - mOffset)
            * getDistanceRatio());
  }
    ...
    if (mInfinite)
      return (int)
          (((getCurrentPositionOffset()
                          + (!mShouldReverseLayout
                              ? position - getCurrentPositionOffset()
                              : -getCurrentPositionOffset() - position))
                      * mInterval
                  - mOffset)
              * getDistanceRatio());
    return (int)
        ((position * (!mShouldReverseLayout ? mInterval : -mInterval) - mOffset)
            * getDistanceRatio());
  }
   ...
   * based on the number of visible pixels in the visible items. This however assumes that all list
   * items have similar or equal widths or heights (depending on list orientation). If you use a
   * list in which items have different dimensions, the scrollbar will change appearance as the user
   * scrolls through the list. To avoid this issue, you need to disable this property.
   *
   * <p>When smooth scrollbar is disabled, the position and size of the scrollbar thumb is based
