...
/** An implementation of {@link ViewPagerLayoutManager} which layouts item in a circle */
...
public class CircleLayoutManager extends ViewPagerLayoutManager {
  ...
  protected float setViewElevation(View itemView, float targetOffset) {
    if (zAlignment == LEFT_ON_TOP) return (540 - targetOffset) / 72;
    else if (zAlignment == RIGHT_ON_TOP) return (targetOffset - 540) / 72;
    else return (360 - Math.abs(targetOffset)) / 72;
  }
  ...
  private static void assertZAlignmentState(int zAlignment) {
    if (zAlignment != LEFT_ON_TOP && zAlignment != RIGHT_ON_TOP && zAlignment != CENTER_ON_TOP) {
      throw new IllegalArgumentException(
          "zAlignment must be one of LEFT_ON_TOP RIGHT_ON_TOP and CENTER_ON_TOP");
    }
  }
